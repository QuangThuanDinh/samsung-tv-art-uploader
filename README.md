...
| `SAMSUNG_TV_ART_TEST_MODE` | `0` | **Diagnostic only.** Changes how a missing `ms.channel.ready` is handled. `0` = normal (time out, drop the socket, retry). `1` = ignore-ready (time out, then proceed as if ready, to test whether the TV accepts Art requests without the handshake). `3` = hang-on-ready (wait forever, reproducing `homebridge-samsung-tizen`). Do not leave enabled in normal operation |
