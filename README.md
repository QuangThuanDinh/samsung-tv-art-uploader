...
> **Upgrading from v0.2.x or v0.3.x?** See the [v0.4.7 release notes](https://github.com/kohlerryan/samsung-tv-art-uploader/releases/tag/v0.4.7) for what's new.
...
Slideshow **Apply** preserves matching cached TV uploads and only replaces images that changed, including matte changes. **ReUpload All Images** uploads the complete replacement set, switches the TV to a new image, and then removes the old uploads.
...
| `SAMSUNG_TV_ART_TEST_MODE` | `0` | **Diagnostic only.** Changes how a missing `ms.channel.ready` is handled. `0` = normal (time out, drop the socket, retry). `1` = ignore-ready (time out, then proceed as if ready, to test whether the TV accepts Art requests without the handshake). `2` = hang-on-ready (wait forever, reproducing `homebridge-samsung-tizen`). `3` is accepted as a deprecated alias for `2`. Do not leave enabled in normal operation |
